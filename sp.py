...
if cases == 'India cases':
    ...
    state_data['lat']=[11.7401,15.9129,28.2180,26.244156,25.0961,30.7333,21.295132,20.1809,28.7041,15.2993,22.309425,29.065773,32.084206,33.7782,23.6102,15.317277,10.850516,34.209515,23.473324,19.663280,24.6637,25.4670,23.1645,20.940920,11.9416,31.1471,27.0238,11.1271,18.1124,23.9408,30.0668,26.8467,22.9868,26.84,22.9868]
    state_data['lon']=[92.6586,79.7400,94.7278,92.537842,85.3131,76.7794,81.828232,73.0169,77.1025,74.1240,72.136230,76.040497,77.571167,76.5762,85.2799,75.713890,76.271080,77.615112,77.947998,75.300293,93.9063,91.3662,92.9376,84.803467,79.8083,75.3412,74.2179,78.6569,79.0193,91.9882,79.0193,80.9462,87.8550,80.94,22.9868]
    ...
    st.subheader('State-wise COVID-19 cases')
    def bar_graph():
...
if cases =='COVID-19 and Heath care':
    ...
    image = Image.open('Data/virus.gif')
    ...
    from PIL import Image
    image = Image.open('Data/flatening the curve.gif')
    ...
    def agegp():
        df = pd.read_csv('Data/AgeGroupDetails.csv')
    ...
    def beds():
        df = pd.read_csv("Data/Hospitals_and_Beds_statewise.csv")
        df.drop(df.tail(1).index,inplace=True)
...
if cases=='Effect on Economy':
    ...
    w_unemp=pd.read_csv('Data/unemp.csv')
    st.title('World unemployment in 2021')
    def unemp():
    ...
    unemp=pd.read_csv('Data/unemp.csv')
